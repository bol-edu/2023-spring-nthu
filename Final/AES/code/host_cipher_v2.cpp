...
int main(int argc, char** argv) {
    ...
    static const int num_CU = 1;
    size_t ARRAY_SIZE = DATA_SIZE*NUM_WORDS*(1024LL*1024/num_CU);
    size_t elements_per_iteration = DATA_SIZE*NUM_WORDS;
    //size_t bytes_per_iteration = elements_per_iteration * sizeof(char);
...
//    fclose(fp);
	...
	if((fp2 = fopen("/mnt/HLSNAS/01.LcglIk/Final/code/data/verify_vec_raw00.dat", "wb")) == NULL){
		printf("Open file Erorr...\n");
	}
    ...
    vector<vector<unsigned char,aligned_allocator<unsigned char>>> roundkey;
    ...
    for(int i=0;i<num_CU;i++){
	    ...
	    vector<unsigned char, aligned_allocator<unsigned char>> roundkey0(roundkey_length);
	    plain.push_back(plain0);
	    cipher.push_back(cipher0);
	    roundkey.push_back(roundkey0);
    }
...
//        			buffer_roundkey[i] = cl::Buffer(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, array_size_bytes_plain, roundkey1.data(), &err));
        ...
        printf("Enqueueing kernel...\n");
        for(int i=0;i<num_CU;i++){
            std::cout << "Running cipher_kernel: " << i << std::endl;
        	OCL_CHECK(err, err = q.enqueueTask(cipher_kernel[i], nullptr, &cipher_events[i]));
            eventList.push_back(cipher_events[i]);
        }
        OCL_CHECK(err, err = cl::Event::waitForEvents(eventList));
		set_callback(cipher_events[i], "cipher_kernel_queue");
...
//    	size_t loop_count = num_CU*iteration_idx;
// 	    printf("Verify Data...\n");
//	    for(int i=0;i<num_CU;i++){
//	    	loop_count++;
//	     	verify(cipher[i],loop_count);
// 	    }
...
//        read_events.clear();
    ...
    size_t loop_count = num_CU*(num_iterations-1);
    printf("Verify Data...\n");
    for(int i=0;i<num_CU;i++){
    	loop_count++;
    	verify(cipher[i],loop_count);
